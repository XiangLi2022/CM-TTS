...
class Preprocessor:
    def __init__(self, preprocess_config, model_config, train_config):
        ...
        self.deal_dataset_type = preprocess_config["dataset"]
    ...
    def build_from_path(self):
        ...
        for i, speaker in enumerate(tqdm(os.listdir(self.in_dir))):
            ...
            for wav_name in tqdm(os.listdir(os.path.join(self.in_dir, speaker))):
                ...
                # print(self.out_dir, "TextGrid", speaker, "{}.TextGrid".format(basename.replace("-","_")))
                if self.deal_dataset_type=="LibriTTS":
                    tg_path = os.path.join(
                        self.out_dir, "TextGrid", basename.split("_")[0], basename.split("_")[1],"{}.TextGrid".format(basename)
                    )
                elif self.deal_dataset_type=="VCTK":
                    tg_path = os.path.join(
                        self.out_dir, "TextGrid", speaker, "{}.TextGrid".format(basename.replace("-","_"))
                    )
                elif self.deal_dataset_type=="LJspeech":
                    tg_path = os.path.join(
                    ...
                    NotImplementedError
                print(tg_path)
                # print("akkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkk")
    ...
    def divide_speaker_by_gender(self, in_dir, speaker_path="speaker-info.txt"):
        if self.deal_dataset_type=="LibriTTS":
            import csv
            speakers = {}
            with open(os.path.join(os.path.dirname(in_dir), "speakers.tsv"), 'r') as file:
                reader = csv.DictReader(file, delimiter='\t')
                for row in reader:
                    speakers[row['READER']] = str(row['GENDER'])
        elif self.deal_dataset_type=="VCTK":
            speakers = dict()
