...
from .karras_diffusion import karras_sample_tts
...
class TrainLoop:
    ...
    def run_loop(self):
        while not self.lr_anneal_steps or self.step < self.lr_anneal_steps:
            batch, cond = next(self.data)  # 数据来源于这个位置
...
class CMTrainLoop(TrainLoop):
    def __init__(
        ...
        **kwargs,
        ...
        super().__init__(**kwargs)  # 这里还传入了本身就需要的参数，这里就包含data
    ...
    def _load_and_sync_target_parameters(self):
        ...
        :return:
        """
        # 如果前面这个不是空，那么优先前面这个，前面为空才是后面这个，后面也为空那么整体为空
        resume_checkpoint = find_resume_checkpoint() or self.resume_checkpoint
        if resume_checkpoint:
            path, name = os.path.split(resume_checkpoint)
            target_name = name.replace("model", "target_model")  # 将其中的"model"换成"target_model"
...
class CMTTSTrainTool(TrainLoop):
    def __init__(
        self,
        *,
        target_model,
        ...
        **kwargs,
        ...
        if target_model:
            ...
            if self.use_fp16:
                ...
                self.target_params = list(self.target_model.parameters())
        ...
        self.model_save_dir = get_blob_logdir()
    ...
    def synthesize_step(self, batch, cond):
        return karras_sample_tts(
    ...
    def run_step(self, batch, cond):
        """
        :param batch:  mel(batch_size,1,seq_len,80)
        :param cond:   dict{
                           conditioner(batch_size, 256, seq_len)
                           speaker_emb(batch_size, 256)
                            }
        :return:
        """
        cm_loss = self.forward_backward(batch, cond)
        took_step = self.mp_trainer.optimize(self.opt)
        if took_step:
            self._update_ema()
            if self.target_model:
    ...
    def _update_target_ema(self):
        ...
        with th.no_grad():
            if self.use_fp16:
                ...
                update_ema(self.target_params, self.mp_trainer.master_params, rate=target_ema)
    ...
    def forward_backward(self, batch, cond):
        self.mp_trainer.zero_grad()
        total_loss = torch.zeros(1).to(batch.device)
        for i in range(0, batch.shape[0], self.microbatch):
            micro = batch[i : i + self.microbatch].to(dist_util.dev())
            micro_cond = {
                k: v[i: i + self.microbatch].to(dist_util.dev())
            ...
            if self.training_mode == "progdist":
                ...
                compute_losses = functools.partial(
                    ...
                    teacher_model=self.teacher_model,
                    teacher_diffusion=self.teacher_diffusion,
                    model_kwargs=micro_cond,  # condition
                )
            elif self.training_mode == "consistency_training":
                compute_losses = functools.partial(
                    self.diffusion.consistency_losses,
            ...
            loss = (losses["loss"] * weights).mean()
            cm_loss = (losses["cm_loss"] * weights).mean()
            with torch.no_grad():
                total_loss += cm_loss
    ...
    def save(self):
        ...
        if dist.get_rank() == 0:
            with bf.BlobFile(
                bf.join(self.model_save_dir, f"opt{step:06d}.pt"),
                "wb",
            ) as f:
                th.save(self.opt.state_dict(), f)
...
def get_blob_logdir():
    ...
    with open("./config/VCTK/train.yaml", 'r') as file:
        train_config = yaml.safe_load(file)
        save_path = osp.join(train_config["path"]["ckpt_path"], "CMDenoiserTTS")
    return save_path
