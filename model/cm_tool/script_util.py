...
import numpy as np
from .tts_net import CMDenoiserTTS, CMTotalTTS
...
def create_model_and_diffusion_tts(
    use_fp16,
    weight_schedule,
    tts_model_config,
    sigma_min=0.002,
    ...
    **kwargs
    ...
    model = CMTotalTTS(use_fp16=use_fp16, **tts_model_config)
...
def create_ema_and_scales_fn(
    target_ema_mode,  # in ["fixed", "adaptive",]
    start_ema,
    scale_mode,  # in ["fixed", "progressive",]
    start_scales,  # 2
    end_scales,
    total_steps,
    ...
    # fixed fixed
    # print("target_ema_mode and scale_mode",target_ema_mode,scale_mode)
