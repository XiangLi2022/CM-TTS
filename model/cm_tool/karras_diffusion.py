...
class KarrasDenoiser:
    def __init__(
        ...
        if len(loss_norm.split("+")) == 2 and loss_norm.split("+")[1] == "mel_loss":
            self.backward_mel_loss = MelLoss()
            loss_norm = loss_norm.split("+")[0]
        else:
            self.backward_mel_loss = None
        # 动态创建需要的loss
        if loss_norm == "lpips":
            self.lpips_loss = LPIPS(replace_pooling=True, reduction="none")
        elif loss_norm == "mel_loss":
    ...
    def print_param(self):
        ...
        print("weight_schedule", self.weight_schedule)
    ...
    def consistency_losses(
        ...
        def euler_solver(samples, t, next_t, x0):
            ...
            if teacher_model is None:
                ...
                denoiser = teacher_denoise_fn(xt, t)
            d = (xt - denoiser) / append_dims(t, dims)
            samples = xt + d * append_dims(next_t - t, dims)
        ...
        tts_total_loss = model.module.get_tts_loss()[0]
        ...
        if self.loss_norm == "l1":
            ...
            loss = mean_flat(diffs) * weights  # 在处理batch维度之外的维度取了一个均值
            ...
            diffs = (distiller - distiller_target) ** 2
            loss = mean_flat(diffs) * weights
        elif self.loss_norm == "mel_loss":
            loss = self.mel_loss(
                mel_targets=distiller_target[:, 0],
                mel_predictions=distiller[:, 0],
                mel_lens=model_kwargs["mel_lens"],
                max_mel_len=distiller.size(2),
            )
        ...
        terms = {}
        print("cm_loss", th.mean(loss), "tts_loss", tts_total_loss)
        terms["cm_loss"] = loss

        if self.backward_mel_loss is not None:
            terms["loss"] = 10 * loss + tts_total_loss + self.backward_mel_loss(
                x_start[:, 0], distiller[:, 0],
                mel_lens=model_kwargs["mel_lens"],
                max_mel_len=distiller.size(2),
            )
        else:
            terms["loss"] = 10 * loss + tts_total_loss
...
def karras_sample_tts(
        diffusion,  # 传进来diffusion
        model,  # 这里我们应该传入那个后半段的模型
        shape,
        steps=2,
        clip_denoised=False,  # 不需要按照图像的准则截断了
        progress=False,
        callback=None,
        model_kwargs=None,
        device=None,
        ...
        sigma_max=80,  # higher for highres?
        rho=7.0,
        sampler="onestep",  # 采用一步的方式进行sample
        s_churn=0.0,
        s_tmin=0.0,
        s_tmax=float("inf"),
        s_noise=1.0,
    ...
    print(
        "steps",steps,
        "\nclip_denoised",clip_denoised,  # 不需要按照图像的准则截断了
        "\nprogress",progress,
        "\ncallback",callback,
        "\nmodel_kwargs",model_kwargs,
        "\ndevice",device,
        "\nsigma_min",sigma_min,
        "\nsigma_max",sigma_max,  # higher for highres?
        "\nrho",rho,
        "\nsampler",sampler,  # 采用一步的方式进行sample
        "\ns_churn",s_churn,
        "\ns_tmin",s_tmin,
        "\ns_tmax",s_tmax,
        "\ns_noise",s_noise,
    ...
    """
    if generator is None:
        generator = get_generator("dummy")  # 应该是一种生成噪声的方法

    # model_kwargs["generator"] = generator
    # model_kwargs["sigma_max"] = sigma_max
    ...
    elif sampler == "our_multistep":
        sampler_args = dict(T=T)
    else:
        sampler_args = {}
...
def stochastic_iterative_sampler(
    ...
    for i in range(len(ts) - 1):
        ...
        x = x0 + generator.randn_like(x) * np.sqrt(next_t ** 2 - t_min ** 2) * 0.85
