...
class Denoiser(nn.Module):
    ...
    def __init__(self, preprocess_config, model_config):
        ...
        multi_speaker = model_config["multi_speaker"]  # 相比于DiffSinger增加了多语者
        ...
        self.residual_layers = nn.ModuleList(
            [   # 相比于DiffSinger增加了多语者，把ResidualBlock重构了
                ResidualBlock(
    ...
    def forward(self, mel, diffusion_step, conditioner, speaker_emb, mask=None):
        ...
        """
        """
        mel torch.Size([48, 1, 80, 587])          mel torch.Size([48, 1, 80, 173])
        diffusion_step torch.Size([48])           diffusion_step torch.Size([48])
        conditioner torch.Size([48, 256, 587])    conditioner torch.Size([48, 256, 173])
        speaker_emb torch.Size([48, 256])         speaker_emb torch.Size([48, 256]) 
        
        print("mel",mel.size())
        print("diffusion_step",diffusion_step.size())
        print("conditioner",conditioner.size())
        print("speaker_emb",speaker_emb.size())
        if mask: print("mask",mask.size())
        """
        x = mel[:, 0]
        # print(x.size()) # [32, 80, *]这个*是没准的
        x = self.input_projection(x)  # x [B, residual_channel, T]
        x = F.relu(x)

        diffusion_step = self.diffusion_embedding(diffusion_step)  # 数学变换，没有可学习参数
        diffusion_step = self.mlp(diffusion_step)  # 对步数进行一个编码
